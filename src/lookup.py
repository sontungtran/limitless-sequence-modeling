...
class LookupTable:
    ...
    def add(self, summarized_tokens, model, optimizer, labels, i, length, order=1, add_new=True,):
    ...
    #     print(f"Total length at order {order}: {sum([x.shape[0] for x in summarized_tokens_hierarchy[order]['summary_tokens'][sum_from:]])}")
        ...
        if sum([x.shape[0] for x in self.summarized_tokens_hierarchy[order]["summary_tokens"][sum_from:]]) > self.n:
    #         print(f"Real Summarizing at {i}, order={order}, sum from {sum_from}")
            ...
            summarized_tokens_next = self.summarize(input_tokens, model, labels, optimizer, i, length)
            self.add(summarized_tokens_next, model, optimizer, labels, i, length, order=order+1, add_new=True)
    ...
    #             print(f"Summarizing at {i}, order={order}, sum from {sum_from}")
                ...
                if summarized_tokens_next == None:
                    ...
                    self.add(summarized_tokens_next, model, optimizer, labels, i, length, order=order+1, add_new=False)
        else:
    ...
    def summarize(self, t, model, labels, optimizer,  i, length):
        if t.shape[0] == 0:
            return None
        ...
        ids = ids.to(self.config['device'], dtype = torch.long) #[C, seq_length]
        mask = mask.to(self.config['device'], dtype = torch.long)
        ...
        torch.nn.utils.clip_grad_norm_(
            parameters=model.parameters(), max_norm=self.config['max_grad_norm']
        )
